...
> **Archived 2026-09-15** from the retired `saas-planner` repo (`docs/superpowers/specs/`). Issue numbers (`#N`) refer to that planner's issues and are kept only for traceability with git history — the planner is no longer used.

**Date:** 2026-06-06
**Status:** Decided
**Spike:** #17 (retired planner)
**Parent epic:** #16 (retired planner)
...
The stack ADR ([#6](https://github.com/oscar-ospina/saas-packages/blob/main/docs/adr/2026-05-27-ds-stack-decision.md)) deliberately left the web framework unchosen and kept `@saas/ui` framework-agnostic. This spike picks it, now that the first surface is concrete: a **Spanish (Colombia) marketing landing** that must be findable (SEO/OG), fast, and accessible, and that will later grow **booking (Agenda)** + **checkout (Pago)** + a **backend** (availability API, payment gateway, webhooks, email).
...
- Scopes story #19 (scaffold): Next.js + TS + Tailwind v4 (`@tailwindcss/postcss`) + the wiring above + `tw-animate-css`, rendering a `@saas/ui` Button end-to-end with CI green.
- Informs story #21 (app shell — App Router routing/layout) and #22 (SEO/metadata/sitemap, `lang="es-CO"`, `next/font`).
...
- Stack ADR — [`2026-05-27-ds-stack-decision.md`](https://github.com/oscar-ospina/saas-packages/blob/main/docs/adr/2026-05-27-ds-stack-decision.md)
