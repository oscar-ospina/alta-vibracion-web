...
- ▶ **In progress — booking & checkout** ([epic #31](https://github.com/oscar-ospina/saas-planner/issues/31)): real in-app *Agenda* (Phase 1 = booking-only via **Google Calendar**, backend in this repo's Route Handlers/Server Actions) then *Pago* (Phase 2). Spikes [#32](https://github.com/oscar-ospina/saas-planner/issues/32) + [#33](https://github.com/oscar-ospina/saas-planner/issues/33) resolved as ADRs. **[#40](https://github.com/oscar-ospina/saas-planner/issues/40) (the Google Calendar server client `lib/calendar.ts` + runbook + smoke-test) is merged** — but its live verification is **⏳ pending owner setup** (see below); the remaining stories #41–#44 (availability, `/agenda` UI, booking action, confirmation) are **blocked on that verification**. Until Agenda lands, the landing routes booking intent to WhatsApp.

> ### ⏳ Pending — live-verify the Google Calendar integration (gates #41–#44)
> #40's code is in, but its defining acceptance criteria are a **live** smoke-test against Liliana's real calendar, which needs owner-provisioned infra that doesn't exist yet. **Do this before building the rest of Agenda:**
