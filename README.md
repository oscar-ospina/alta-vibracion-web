...
- Payment instructions on the booking-created panel and the status page while pending: the Bre-B key from `PAYMENT_BREB_KEY` (never a bank account number), the amount and the booking code as reference. Confirmation stays manual. The public horizon is 30 days. Liliana can create a booking by hand from `/admin` (any Colombia time, confirmed or pending, same double-booking guard) for late payments, gift redemptions and WhatsApp clients.
- Gift orders (`gift_orders` table, `lib/gifts.ts`): Liliana creates an order from the inquiry or from scratch in `/admin/gifts` (buyer, price, optional campaign price when the campaign allows gifts, message, conditions), marks it paid after seeing the transfer, and shares the invitation `/regalar/<code>`. The beneficiary accepts there and books on `/agenda?bono=<code>` with their own data: the booking is confirmed on creation, price 0, and the voucher is spent (partial unique index on `bookings.gift_order_id`). The sale is recorded once, on the order. `/admin/gifts` lists "Regalos por agendar" and warns when paid vouchers outnumber the free slots of the next 30 days (a warning, never a block). A manual booking can redeem a voucher by its code.
- Handoff (plan section 12, row 6): `/admin` has a pause switch (public bookings closed, manual path shown, admin untouched), CSV exports of bookings, interests, campaigns and gifts at `/admin/export/<table>` (Basic auth, Colombia time, BOM for Excel), and the operating guide at `/admin/guide` (`content/admin/operations.md`: the five operations, what Liliana changes without code and what needs Óscar, what to do when something fails). Deferred: survey, newsletter, payment gateway, the pre-session form itself (external until the legal texts are signed).
...
```
app/            routes (home, yo, yo/[slug], nosotros, nosotros/[slug], celebremos, empresas, regalar, regalar/[codigo], encuentros/[codigo], agenda, agenda/[code], admin, admin/bookings/[id], admin/interests, admin/campaigns, admin/gifts, admin/script, contact, terms, privacy), server actions (agenda/, interests/, admin/)
...
lib/            site.ts, catalog.ts (services, lines, statuses), contact.ts, interests.ts, campaigns.ts, gifts.ts, payment.ts, agenda/ (time, availability, bookings, delivery), admin-auth
