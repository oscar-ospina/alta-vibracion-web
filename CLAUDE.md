...
- `settings.bookings_paused` closes the public agenda and `createBooking`; the admin and manual bookings ignore it. `/admin/export/<table>` is the only path personal data leaves the database, behind the same Basic-auth check as every admin action.
